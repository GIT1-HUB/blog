...
export default {
  ...
  methods: {
      ...
      if (this.$store.state.username == "登录/注册") {
        ...
        if (status == 200 && error == 0) {
          for (let i = 0; i < followers.length; i++) {
            if (vuexfollweds.includes(followers[i].username)) {
              followers[i].followStatus = true;
            } else {
              followers[i].followStatus = false;
            }
          }
    ...
    async handleClick(curname, index) {
      //  console.log(this.followers[index].followStatus)
      if (this.$store.state.username == "登录/注册") {
        this.model = true;
        ...
        if (this.followers[index].followStatus == false) {
          ...
          if (status == 200 && error == 0) {
            this.$set(this.followers[index], `followStatus`, true);
          }
        } else {
          let json = { currentUser: this.username, followedUser: curname };
          let {
          ...
          if (status == 200 && error == 0) {
            this.$set(this.followers[index], `followStatus`, false);
          }
        }
      }
    }
