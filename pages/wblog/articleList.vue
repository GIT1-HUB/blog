  ...
  methods: {
    ...
    async initFront(page) {
      try {
        ...
        let {
          data: { count, front }
        } = await this.$axios.get(`${baseurl}/article/frontList`, {
          params: { page, pagesize: 5, author: this.$store.state.username }
        });
        this.front = front;
        // console.log(front)
        // console.log(123)
    ...
    async initBack(page) {
      try {
        ...
        let {
          data: { count, back }
        } = await this.$axios.get(`${baseurl}/article/backList`, {
          params: { page, pagesize: 5, author: this.$store.state.username }
        });
        this.back = back;
    ...
    searchArticle() {
      ...
      if (this.searchStr == "") {
        ...
        // 搜索前端
        this.$axios
          .get(`${baseurl}/article/searchFrontArticleListByAuthor`, { params: json })
          .then(res => {
        ...
        // 搜索后端
        this.$axios
          .get(`${baseurl}/article/searchBackArticleListByAuthor`, { params: json })
          .then(res => {
            if (res.status == 200 && res.data.error == 0) {
